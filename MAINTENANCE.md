...
Baseline history: `TestOpenAICompatExecutorToolResultContentByInputModalities` and the Meta User-Agent fixture previously failed identically on untouched upstream and the fork. Both were resolved upstream and pass as of the 2026-09-17 sync at upstream `b773607e3e7756dc6020a291825e4eb08899595a`. There is no accepted failing baseline now: `go test ./...` must pass fully. Any new failure blocks promotion. If a future inherited failure appears, reproduce it on an untouched upstream checkout first, record the exact upstream SHA here, and report it separately only while unchanged-source proof, routing/quota tests, build, and live Codex/Claude canaries pass.
